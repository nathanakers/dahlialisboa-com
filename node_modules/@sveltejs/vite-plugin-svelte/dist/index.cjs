...
function resolveViaPackageJsonSvelte(importee, importer) {
  if (importer && isBareImport(importee)) {
    const importeePkgFile = import_require_relative.default.resolve(`${importee}/package.json`, import_path2.default.dirname(importer));
    const importeePkg = JSON.parse(import_fs3.default.readFileSync(importeePkgFile, { encoding: "utf-8" }));
    if (importeePkg.svelte) {
      return import_path2.default.resolve(import_path2.default.dirname(importeePkgFile), importeePkg.svelte);
    }
  }
}
function isBareImport(importee) {
  if (!importee || importee[0] === "." || importee[0] === "\0" || import_path2.default.isAbsolute(importee)) {
    return false;
  }
